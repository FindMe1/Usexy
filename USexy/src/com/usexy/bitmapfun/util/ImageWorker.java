...
public abstract class ImageWorker {
	...
	protected ImageWorker(Context context, int width, int height) {
	...
	/**
	 * 加载图片给一个imageView Load an image specified by the data parameter into an
	 * ImageView (override {@link ImageWorker#processBitmap(Object)} to define
	 * the processing logic). A memory and disk cache will be used if an
	...
	public void loadImage(Object data, ImageView imageView,
		...
		if (bitmap != null) {
			...
			if (loadImageCallBack != null) {
				loadImageCallBack.loadImageCompleted(imageView, bitmap);
			}
		} else if (cancelPotentialWork(data, imageView)) {
			final BitmapWorkerTask task = new BitmapWorkerTask(imageView,loadImageCallBack);
			final AsyncDrawable asyncDrawable = new AsyncDrawable(mResources,
					mLoadingBitmap, task);
			imageView.setImageDrawable(asyncDrawable);
	...
	public void setLoadingImage(int resId) {
		mLoadingBitmap = ImageResizer.decodeSampledBitmapFromResource(
				mResources, resId, width, height);
	}
	 ...
	 */
	protected abstract Bitmap processBitmap(Object data,LoadImageCallBack loadImageCallBack);
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
		...
		public BitmapWorkerTask(ImageView imageView,LoadImageCallBack loadImageCallBack) {
		...
		protected Bitmap doInBackground(Object... params) {
			...
			if (bitmap == null && !isCancelled()
					&& getAttachedImageView() != null && !mExitTasksEarly) {
				bitmap = processBitmap(params[0],loadImageCallBack);
			}
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			if (bitmap != null && imageView != null) {
				...
				setImageBitmap(imageView, bitmap,loadImageCallBack);
	 ...
	 * @param bitmap
	 */
	private void setImageBitmap(ImageView imageView, Bitmap bitmap,LoadImageCallBack loadImageCallBack) {
		if (mFadeInBitmap) {
