...
public class ImageCache {
    ...
    /**是否需要内存缓存，默认不使用内存缓存，如果使用默认给出1M,内存缓存大小有多方面因素决定，>memory class * 0.05,<memory class * 0.05 */
    private static final boolean DEFAULT_MEM_CACHE_ENABLED = false;
    /**是否需要sdcard缓存，默认给出10M*/
 ...
 /*       if (Utils.hasFroyo()) {
        ...
        final String cacheDir = "/GYsuning/data/" + context.getPackageName() + "/cache/";
    ...
    public static long getUsableSpace(File path) {
        ...
        final StatFs stats = new StatFs(path.getPath());//一个模拟linux的df命令的一个类,获得SD卡和手机内存的使用情况 
